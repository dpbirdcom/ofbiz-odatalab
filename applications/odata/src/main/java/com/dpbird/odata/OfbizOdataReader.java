...
//    public Entity findRelatedEntityByKey(Entity entity,EdmNavigationProperty edmNavigationProperty,
//                                                                   EntityCondition primaryKeyCondition) throws OfbizODataException {
//        //查询子对象数据
//        EntityCollection relatedEntityCollection = findRelatedEntityCollection(entity, edmNavigationProperty, null);
//        for (Entity relatedEntity : relatedEntityCollection.getEntities()) {
//            OdataOfbizEntity ofbizEntity = (OdataOfbizEntity) relatedEntity;
//            GenericValue entityGeneric = ofbizEntity.getGenericValue();
//            //只返回符合条件的数据
//            List<GenericValue> filterResult = EntityUtil.filterByCondition(UtilMisc.toList(entityGeneric), primaryKeyCondition);
//            if (UtilValidate.isNotEmpty(filterResult)) {
...
//                    (EdmBindingTarget) edmParams.get("edmBindingTarget"), keyMap);
//            if (genericValue == null) {
//                throw new OfbizODataException(HttpStatus.SC_NOT_FOUND + "", "Entity not found: " + entityName);
//            }
//            entity = makeEntityFromGv(genericValue);
